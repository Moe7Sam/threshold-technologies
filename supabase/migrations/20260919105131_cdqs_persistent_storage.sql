/*
# CDQS Persistent Storage — Applications, Certificates, and Professional Registry

## Purpose
Creates the complete database backend for the CDQS programme and Threshold
Professional Registry. The frontend in lib/cdqsStore.ts and lib/registryStore.ts
already calls these exact tables and RPC functions via the Supabase REST API.

## New Tables
1. cdqs_admin_settings — singleton storing bcrypt-hashed admin PIN
2. cdqs_applications — public CDQS programme applications
3. cdqs_certificates — issued certificates with verification tokens
4. cdqs_registry_profiles — professional registry member profiles
5. cdqs_registry_evidence — evidence items submitted for review

## Security
- RLS enabled on all tables
- Public can INSERT applications only (no direct SELECT)
- All reads go through SECURITY DEFINER functions gated by admin PIN or profile token
- Admin PIN checked via bcrypt hash (extensions.crypt / extensions.gen_salt)

## RPC Functions
- cdqs_admin_pin_valid, cdqs_admin_list_applications, cdqs_admin_create_certificate
- cdqs_verify_certificate (public)
- cdqs_admin_accept_application, cdqs_registry_get_profile (public)
- cdqs_registry_list_evidence (public), cdqs_registry_submit_evidence (public)
- cdqs_admin_list_registry_profiles, cdqs_admin_list_evidence
- cdqs_admin_review_evidence, cdqs_admin_approve_registry_profile

## Notes
- Default admin PIN: 'threshold2026'
- pgcrypto functions are in the 'extensions' schema on this Supabase project
*/
...
values (true, extensions.crypt('threshold2026', extensions.gen_salt('bf')))
...
-- -----------------------------------------------------------------------
-- 7. Policies — public can INSERT applications only (no direct SELECT)
-- -----------------------------------------------------------------------
...
-- 8a. Admin PIN validator (internal — revoked from public)
...
-- 8b. Admin: list all applications
...
begin
  ...
  return query
  select
    a.id, a.full_name, a.email, a.phone, a.role, a.company,
    a.years_experience, a.city, a.goal, a.payment_preference,
    a.status, a.revenue_aed, a.created_at
...
-- 8c. Admin: create/update certificate
...
begin
  ...
  insert into public.cdqs_certificates (
    certificate_number, student_name, role_title, cohort_name,
    issue_date, verifier_name
  )
  values (
    p_certificate_number, p_student_name, p_role_title, p_cohort_name,
    p_issue_date, p_verifier_name
  )
...
-- 8d. Public: verify certificate by token
...
as $$
  select
    c.id, c.certificate_number, c.student_name, c.role_title,
    c.cohort_name, c.issue_date, c.verifier_name,
    c.verification_token, c.status
...
-- 8e. Admin: accept application → create registry profile
...
begin
  ...
  insert into public.cdqs_registry_profiles (
    application_id, full_name, email, title, company,
    location, certification, cohort, status, registration_number
  )
  select
    ...
    'CDQS', 'CDQS Cohort 001', 'candidate', next_registration
  from public.cdqs_applications a
  where a.id = p_application_id
...
-- 8f. Public: get registry profile by token
...
as $$
  select
    p.id, p.application_id, p.full_name, p.email, p.title,
    p.company, p.location, p.linkedin, p.certification, p.cohort,
    p.status, p.registration_number, p.profile_token,
    p.issued_at, p.valid_until
...
-- 8g. Public: list evidence for a profile
...
as $$
  select
    e.id, e.profile_id, e.title, e.category,
    e.file_name, e.notes, e.status, e.submitted_at
...
-- 8h. Member: submit evidence item
...
begin
  ...
  insert into public.cdqs_registry_evidence (
    profile_id, title, category, file_name, notes
  )
  values (
    target_profile_id, p_title, p_category, p_file_name, p_notes
  )
  returning
...
-- 8i. Admin: list all registry profiles
...
-- 8j. Admin: list evidence for a specific profile
...
-- 8k. Admin: review an evidence item
...
-- 8l. Admin: approve a registry profile (set to verified)
